...
public:
  Piece(PieceColor pieceColor, u_int32_t row, u_int32_t col)
      : threatened(false), color(pieceColor), location(row, col), moveCount(0),
        schema() {}
   ...
   */
  virtual void threat() noexcept { threatened = !threatened; }

  /*
   * getter for threatened field
   */
  virtual bool isThreatened() const noexcept { return threatened; }

  /*
   * setter for PieceColor field
   */
  virtual void setColor(PieceColor pieceColor) noexcept { color = pieceColor; }
  ...
  virtual u_int32_t getMoveCount() noexcept { return moveCount; }
