...
Location::Location(const Location &rhs) noexcept
    : row(rhs.row), column(rhs.column) {}

Location::Location(u_int32_t row, u_int32_t col) noexcept
    : row(row), column(col) {}
...
const u_int32_t &Location::getRow() const { return row; }

const u_int32_t &Location::getColumn() const { return column; }
